...
# HTML elements that should have closing tags even if empty
...
class HTMLGenerator(ASTVisitor):
    ...
    def visit_element(self, node: Element) -> Any:
        ...
        # Check if void element or has children
        is_void = node.tag_name.lower() in VOID_ELEMENTS
        has_children = len(node.children) > 0
        needs_closing = node.tag_name.lower() in ELEMENTS_WITH_CLOSING_TAG
        
        if is_void and not has_children:
            self.output.append(" />")
        elif not has_children and not needs_closing:
            self.output.append(" />")
        else:
            self.output.append(">")
    ...
    def visit_import(self, node: Import) -> Any:
        ...
        # Output in a format that can be resolved later
        if node.params:
