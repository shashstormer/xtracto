...
class LogLevel(IntEnum):
    ...
    TRACE = 5       # Most verbose: every token, every parse step
    DEBUG = 10      # Detailed: major parsing decisions
    INFO = 20       # Normal: file processing, build progress
    WARNING = 30    # Issues that don't prevent completion
    ERROR = 40      # Errors that may affect output
...
# Legacy Log class for backward compatibility
class Log:
    ...
    def get_logger(config_path=None):
        ...
        try:
            ...
            # Fall back to new logger if requestez not available
            return get_logger()
